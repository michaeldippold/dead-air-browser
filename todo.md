...
## NEXT SESSION — Tutorial Infrastructure

> Unit travel time (hop-distance BFS, `state.transits`, decoupled `resolveTransits()`, TRAVELING
> panel with siren-dot countdowns, click-to-reopen detail view showing EN ROUTE) shipped and was
> verified live in-browser — dispatch → countdown → siren → arrival → COMMS line, no console
> errors. See "What's Shipped" above. What's left is the tutorial layer built on top of it: the
> window-manager scripting API, the `onEnter` script hook, the START MISSION trigger, and the
> tutorial colleague's content. The four existing scripted characters stay as-is for now
> (deferred, see Scenario System section). Read design.md's "Movement & Risk" section for the why.

### 1. Window-manager scripting API
...
### 2. Script node `onEnter` side-effect field
...
### 3. Tutorial trigger point — before the game even starts
